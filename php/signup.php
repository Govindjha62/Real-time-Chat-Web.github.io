...
if(!empty($fname) && !empty($lname) && !empty($email) && !empty($password)){
    if(filter_var($email, FILTER_VALIDATE_EMAIL)){//if email is valid
        ...
        $sql = mysqli_query($conn, "SELECT email FROM users WHERE email= '{}'");
        if(mysqli_num_rows($sql) > 0){//if email alredy exist
            ...
            if(isset($_FILES['image'])){//if file is uploaded
                ...
                // let's emplode image and get the last extesion like jpg, jpeg and png
                $img_explode = explode('.', $img_name);
                $img_ext = end($img_explode);//here we get the extension of an user upload img file

                $extensions = ['png', 'jpeg', 'jpg'];//these are some valid img ext and we've store them in erray
                if(in_array($img_ext, $extensions) == true){//if user uploaded img text is matched with any array extensions
                    $time = time();// this will return us current time...
                    //we need this time because when you uploading user img to in our folder we rename user file with current time
                    // so all image file will have a unique name
                    ...
                    echo "Please select an Image and Image file - jpeg, jpg, png!";
                }
